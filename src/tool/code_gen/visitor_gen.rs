...
pub fn generate_visitor(vocabulary: &Vocabulary) {
  ...
  vocabulary.non_terminal_name_to_id.iter().for_each(|(name, index)| {
    rules.push(RuleRenderContext { items: Vec::new(),
      pascal: pascal(&name), underscore: name.to_owned(), upcase: name.to_ascii_uppercase(), rule_index: *index });
  }); 
